...
def signup():
    ...
    if signup_form.validate_on_submit():
        try:
            ...
            db.session.commit()
            flask.flash("Account created successfully.", "info")
        except sqlalchemy.exc.IntegrityError:
            flask.flash("You've already signed up. Try Sign-in.", 'info')
...
def signin():
    ...
    if signin_form.validate_on_submit():
        ...
        if req_user:
            if check_password_hash(req_user.password,
        ...
        flask.flash("Incorrect email or password.", "error")
...
@login_required
def download():
    # Flushing flash messages, so that it won't disturb other pages.
